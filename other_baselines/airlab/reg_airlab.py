...
# sys.path.append("/media/data/robert/code/airlab")
import os
...
from TPTBox import NII, POI, Centroids, Image_Reference, calc_centroids, to_nii_seg
from TPTBox.core import sitk_utils

# """The pipeline performs per-vertebra atlas-to-target registration using an initial rigid alignment optimized with normalized cross-correlation,
# followed by a diffeomorphic B-spline deformable registration optimized with mean squared error and strong diffusion regularization; segmentations
# are warped with nearest-neighbor interpolation, centroids are transformed consistently via the displacement field, and label integrity is enforced
# through crop-based processing and post-registration morphological filling."""
...
def load_atlas_individual(atlas_path, vert_nii: NII, label: int):
    ...
    if poi_path2 is not None and poi_path2.exists() and not poi_path.exists():
        ...
        ctd.save(poi_path)
    poi = POI.load(poi_path) if poi_path.exists() else calc_centroids(subreg_atlas)
...
def load_atlas_file(atlas_path: Atlas, vert_nii: NII, label: int):
    ...
    if atlas_path.poi_atlas is None:
        ...
        poi = atlas_path.poi_atlas.extract_subregion(label)
        # TODO
...
def __make_subreg_by_registration(
    ...
    # Crop Atlas
    ex_slice_moving = moving_atlas.compute_crop(dist=1, maximum_size=fixed_nii.shape)
    ...
    poi.apply_crop_(tuple(slice(x) for x in subreg_atlas.GetOrigin()))
    ...
    poi.apply_crop_(tuple(slice(-x.start) for x in ex_slice_fixed)) if poi is not None else None
...
def make_subreg_by_registration(
    ...
    for x in out:
        transformed_mask, poi, ex_slice_fixed = x  # type: ignore
        print(poi)
...
def make_atlas_from_sample(
    ...
    if vert_subreg_atlas_file is not None:
        ...
        assert vert_atlas.shape == subreg_atlas.shape if subreg_atlas is not None else True, (
    ...
    for x in out:
        ...
        if poi_out is None:
            ...
            for k, v in poi.items():
                poi_out[k] = v
        print(poi)
        result_volume[ex_slice_fixed] += transformed_mask.set_dtype(result_volume.dtype).get_seg_array()
