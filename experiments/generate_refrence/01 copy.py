...
def run(moving_img: NII, target: NII, mirror, gpu=0):
    if mirror:
...
def run_all(folder: Path, target: NII, out_folder: Path, min_delta=min_delta, save_seg=False):
    print(f"{min_delta=}")
    for i in (folder).iterdir():
        ...
        reg = run(moving_img, target, mirror)
        ...
        if save_seg:
            out = out_folder / (i.name.split(".")[0] + ".nii.gz")
            atlas_reg = reg.transform_nii(moving_img.extract_label(leg_ids, True)).save(out)
        # print(mirror)
...
if __name__ == "__main__":
    ...
    run_all(out_mrk, target, out_folder, save_seg=True)
