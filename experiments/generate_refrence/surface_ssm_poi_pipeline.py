...
def infer_pois(ssm, new_seg_path, landmark_template, out_file):
...
if __name__ == "__main__":
    ...
    # SSM
    ssm = train_surface_ssm(train_segmentations[:-1])

    infer_pois(
        ssm,
        train_segmentations[-1],
        template_poi,
        out_voting / "treg" / f"{train_segmentations[-1].name.split('.')[0]}_predicted_landmarks.mrk.json",
    )
    # TODO Next step revert the landmark
