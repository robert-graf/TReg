...
def compute_angles(poi_original: POI_Global, out_poi=None, out_poi_orthosys=None):
    ...
    FADP = np.array(poi[POI_MAP["FADP"]])
    FAAP = np.array(poi[POI_MAP["FAAP"]])
    ...
    TTP = np.array(poi[POI_MAP["TTP"]])

    # Trochlea / groove
    TGCP = np.array(poi[POI_MAP["TGCP"]])
    TGPP = np.array(poi[POI_MAP["TGPP"]])

    TRLP = np.array(poi[POI_MAP["TRLP"]])
    TRMP = np.array(poi[POI_MAP["TRMP"]])

    # Patella
    PPP = np.array(poi[POI_MAP["PPP"]])
    PDP = np.array(poi[POI_MAP["PDP"]])

    PMP = np.array(poi[POI_MAP["PMP"]])
    PLP = np.array(poi[POI_MAP["PLP"]])

    PRHP = np.array(poi[POI_MAP["PRHP"]])
    PRPP = np.array(poi[POI_MAP["PRPP"]])
    PRDP = np.array(poi[POI_MAP["PRDP"]])

    angles = {}

    angle_lines: list[MKR_Lines] = [
        {"key_points": [(99, 1), (99, 2)], "color": [1, 0.0, 0.0]},
        {"key_points": [POI_MAP["FHC"], (99, 4)], "color": [1, 0.0, 0.0]},
        {"key_points": [(99, 3), (99, 4)], "color": [1, 0.0, 0.0]},
    ]
    ...
    angle_lines.append({"key_points": [POI_MAP["TLCP"], POI_MAP["TMCP"]], "color": color_from_idx(idx)})
    idx += 1
    ...
    angles["LPFA"] = angle_deg(v1, v2)

    angle_lines.append({"key_points": [POI_MAP["TTP"], POI_MAP["FHC"]], "color": color_from_idx(idx)})
    angle_lines.append({"key_points": [POI_MAP["FNP"], POI_MAP["FHC"]], "color": color_from_idx(idx)})
    idx += 1
    ...
    angles["MPFA"] = angle_deg(v1, v2)

    angle_lines.append({"key_points": [POI_MAP["FHC"], POI_MAP["TTP"]], "color": color_from_idx(idx)})
    angle_lines.append({"key_points": [POI_MAP["FADP"], POI_MAP["FAAP"]], "color": color_from_idx(idx)})
    ...
    angles["NSA"] = angle_deg(v1, v2)

    angle_lines.append({"key_points": [POI_MAP["FHC"], POI_MAP["FNC"]], "color": color_from_idx(idx)})
    angle_lines.append({"key_points": [POI_MAP["FADP"], POI_MAP["FAAP"]], "color": color_from_idx(idx)})
    ...
    angles["sagittal_HKA"] = angle_deg(v3, v1) + angle_deg(v3, v2)

    angle_lines.append({"key_points": [POI_MAP["FHC"], POI_MAP["FNP"]], "color": color_from_idx(idx)})
    angle_lines.append({"key_points": [POI_MAP["TAC"], POI_MAP["TKC"]], "color": color_from_idx(idx)})
    angle_lines.append({"key_points": [POI_MAP["TMCA"], POI_MAP["TMCP"]], "color": color_from_idx(idx)})
    ...
    angles["patella_tilt"] = angle_deg(v1, v2)

    angle_lines.append({"key_points": [POI_MAP["PMP"], POI_MAP["PLP"]], "color": color_from_idx(idx)})
    angle_lines.append({"key_points": [POI_MAP["FMCP"], POI_MAP["FLCP"]], "color": color_from_idx(idx)})
    ...
    phi = angle_deg(v1, v2)
    angles["trochlea_depth_lateral"] = np.sin(np.deg2rad(phi)) * np.linalg.norm(v1) * 1000

    v1 = TGCP - TRMP
    v2 = TRMP - TRLP

    phi = angle_deg(v1, v2)
    angles["trochlea_depth_medial"] = np.sin(np.deg2rad(phi)) * np.linalg.norm(v1) * 1000

    v1 = TRLP - TGCP
    v2 = TGCP - TRMP

    angles["trochlea_angle"] = 180 - angle_deg(v1, v2)

    angle_lines.append({"key_points": [POI_MAP["TRLP"], POI_MAP["TGCP"]], "color": color_from_idx(idx)})
    angle_lines.append({"key_points": [POI_MAP["TRMP"], POI_MAP["TGCP"]], "color": color_from_idx(idx)})
    idx += 1

    #### FIXME lost PDFA_sagittal_2D ###
    # idx += 1
    # v1 = FHC - FNP
    # v2 = FMCPC - FLCPC
    #
...
if __name__ == "__main__":
    ...
    a, b, c = compute_angles(poi, out_poi="/media/data/robert/code/TReg/data/lines2.mrk.json")
    print(a)
