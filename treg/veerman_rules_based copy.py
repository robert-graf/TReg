...
def run_single_case(nii, stl_folder: "Path | str", side: Literal["R", "L"], output_csv=None):
    ...
    poi = nii.make_empty_POI().to_global()
    ...
    warnings = []
    # load_save_stls(nii, results, stl_folder, side)
    results = {}
    compute_points(nii, poi, stl_folder, side, warnings)
    step_3(poi)
    step_4(poi)

    if output_csv:
        step_5(results, output_path=output_csv)
    ...
    global_poi.to_local(nii).save(stl_folder / "poi.json")
